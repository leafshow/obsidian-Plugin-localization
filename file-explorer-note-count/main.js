    ...
    this.handler = (...args) => {
      ...
      for (const arg of args) {
        ...
        this.waitingList.push(
          (_a = getParentPath(path)) !== null && _a !== void 0 ? _a : "/",
        );
      }
      this.update();
    };
...
class FENoteCountSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(containerEl)
      .setName("添加根文件夹")
      .setDesc(
        "默认情况下，黑曜石没有提供根文件夹。它被移动到下拉菜单，在库之间切换。 " +
          "如果您想在文件资源管理器中查看根文件夹及其计数，请启用此选项",
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.addRootFolder)
          .onChange((value) => {
    ...
    if (!this.showOnlyNoteValue) {
      new obsidian.Setting(this.containerEl)
        ...
        .addTextArea((text) => {
          ...
          text.setPlaceholder("将其留空以计算所有类型的文件");
          text
  ...
  constructor() {
    ...
    this.initialize = (revert = false) => {
      ...
      const getViewHandler = (revert) => (view) => {
        ...
        if (!revert) {
          this.registerEvent(
            this.app.workspace.on("css-change", this.setupRootFolder),
          );
          ...
          this.register(
            around(view, {
              onClose: (next) =>
                function () {
                  setTimeout(
                    () => doWithFileExplorer(plugin, getViewHandler(false)),
                    1e3,
                  );
    ...
    this.setupRootFolder = (revert = false) => {
      ...
      const root =
        (_c =
          (_b =
            (_a = this.fileExplorer) === null || _a === void 0
              ? void 0
              : _a.fileItems) === null || _b === void 0
            ? void 0
            : _b["/"]) !== null && _c !== void 0
          ? _c
          : null;
